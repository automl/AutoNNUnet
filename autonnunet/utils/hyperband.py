...
def compute_hyperband_budgets(
        ...
        b_max: int,
        eta: int
        ) -> tuple[dict[int, list], dict[int, list], int, int]:
    ...
    for i in range(s_max + 1):
        ...
        c_and_b = SuccessiveHalving._compute_configs_and_budgets_for_stages(
            eta, b_max, max_iter, s_max
        )
        budgets_in_stage[i], n_configs_in_stage[i] = c_and_b

        budgets_in_stage[i] = [round(b) for b in budgets_in_stage[i]]

    total_budget = 0
    for i in range(s_max + 1):
        for n_configs, budget in zip(
    ...
    for i in range(s_max + 1):
        configs_list = "[" + ", ".join(
            [str(c).rjust(4) for c in n_configs_in_stage[i]]) + "]"
        budgets_list = "[" + ", ".join(
            [str(b).rjust(4) for b in budgets_in_stage[i]]) + "]"
